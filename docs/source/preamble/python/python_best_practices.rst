...
the file must be interpreted by Python (and which Python) is the general way to run a script
...
Another way is to execute it directly. This can be done with
...
If we remove the shebang and try to execute the script, it will return the following errors, because it doesn't know
...
When a module is run directly, it's :code:`__name__` property will be :code:`'__main__'`.
...
The :code:`Exception` clause is our example is very broad, but a **MUST** in code that is still under development.
